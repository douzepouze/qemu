...
#define DPRINTF(fmt, ...)                                   \
do {                                                        \
    fprintf(stderr, "flash: " fmt , ## __VA_ARGS__);       \
} while (0)
#else
...
static uint64_t nvmc_read(void *opaque, hwaddr offset, unsigned int size)
{
    Nrf51NVMCState *s = NRF51_NVMC(opaque);
    uint64_t r = 0;
/*
...
static void nvmc_write(void *opaque, hwaddr offset, uint64_t value,
        unsigned int size)
{
    Nrf51NVMCState *s = NRF51_NVMC(opaque);
/*
...
static void nrf51_nvmc_realize(DeviceState *dev, Error **errp)
{
    ...
    // TODO put link properties for NVMs here
}
...
static uint64_t code_read(void *opaque, hwaddr offset, unsigned int size)
{
    Nrf51CODEState *s = NRF51_CODE(opaque);
    uint64_t r = 0;
    uint8_t *p;
...
static void code_write(void *opaque, hwaddr offset, uint64_t value,
        unsigned int size)
{
    Nrf51CODEState *s = NRF51_CODE(opaque);
    uint8_t *p;
    ...
    if((offset & 0x3) != 0x00) {
        ...
        "%s: unaligned write to 0x%" HWADDR_PRIx " shall cause hard fault\n",
        __func__, offset);
        return;
    }

    if(offset >= s->code_size * PAGE_SIZE) {
        qemu_log_mask(LOG_GUEST_ERROR,
        "%s: out of bound write access 0x%" HWADDR_PRIx "\n",
        __func__, offset);
        return;
    }
...
static const MemoryRegionOps code_ops = {
        .read = code_read,
        .write = code_write,
        .endianness = DEVICE_LITTLE_ENDIAN,
};

static void nrf51_code_init(Object *obj)
{
    Nrf51CODEState *s = NRF51_CODE(obj);
    SysBusDevice *sbd = SYS_BUS_DEVICE(obj);
}

static void nrf51_code_realize(DeviceState *dev, Error **errp)
{
    Nrf51CODEState *s = NRF51_CODE(dev);
    Error *local_err = NULL;

    if(!s->code_size) {
        error_setg(errp, "code size property was not set");
        return;
    }

    /* TODO put link properties for NVMs here*/

    memory_region_init_rom_device(&s->mem, OBJECT(dev), &code_ops, s,
        TYPE_NRF51_CODE, s->code_size * PAGE_SIZE, &local_err);
    if (local_err) {
...
static void nrf51_code_unrealize(DeviceState *dev, Error **errp)
{
    Nrf51CODEState *s = NRF51_CODE(dev);
}

static Property nrf51_code_properties[] = {
    DEFINE_PROP_UINT32("code_size", Nrf51CODEState, code_size, 0),
    DEFINE_PROP_END_OF_LIST(),
};

static void nrf51_code_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->props = nrf51_code_properties;
    dc->realize = nrf51_code_realize;
    dc->unrealize = nrf51_code_unrealize;
}

static const TypeInfo nrf51_code_info = {
    .name = TYPE_NRF51_CODE,
    .parent = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(Nrf51CODEState),
    .instance_init = nrf51_code_init,
    .class_init = nrf51_code_class_init
};

static void nrf51_code_register_types(void)
{
    type_register_static(&nrf51_code_info);
}

type_init(nrf51_code_register_types)
